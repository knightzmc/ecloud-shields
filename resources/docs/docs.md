...
The `home-routes` handler in the `ecloud-shields.routes.home` namespace
...
The `home-page` function will in turn call the `ecloud-shields.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `ecloud-shields.handler` namespace:
...
Request middleware functions are located under the `ecloud-shields.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `ecloud-shields.dev-middleware` namespace found in
